$City::Demerits::Taser = 5;

if(!isObject(taserItem))
{
	AddDamageType("Taser",   '<bitmap:Add-Ons/GameMode_City/CityItems/ci/taser> %1',    '%2 <bitmap:Add-Ons/GameMode_City/CityItems/ci/taser> %1', 0.5, 1);
	
	datablock AudioProfile(taserExplosionSound)
	{
		filename	 = "Add-Ons/GameMode_City/sounds/radioWaveExplosion.wav";
	...
	datablock ItemData(taserItem)
	{
		...
		shapeFile = "Add-Ons/GameMode_City/CityItems/weapons/taser.dts";
		rotate = false;
		mass = 1;
		density = 0.2;
		elasticity = 0.2;
		...
		uiName = "Taser";
		iconName = "Add-Ons/GameMode_City/CityItems/ItemIcons/taser";
		doColorShift = false;
...
function taserProjectile::damage(%this, %obj, %col, %fade, %pos, %normal)
{
	...
	if(%col.getType() & $TypeMasks::PlayerObjectType)
	{
		...
		%col.setVelocity(VectorScale((1 + getRandom()) SPC (1 + getRandom()) SPC "1", 10));
		tumble(%col);
		if(isCityObject(%colClient = %col.client) && isObject(%client = %obj.client))
			if(!%client.City_CanLegallyDamage(%colClient))
			{
				%client.centerPrint("\c6You have commited a crime. [\c3Tasing Innocents\c6]", 2);
				%client.addDemerits($City::Demerits::Taser);
			}
	}
}